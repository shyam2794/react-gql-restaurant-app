...
class DayWiseBill extends React.Component {
  ...
  dayWiseBill(history, monthdata) {
    ...
    let dayWiseBill = historyCopy.filter(
      value => value.month === monthdata.month && value.year === monthdata.year
    );
      ...
      <div>
        ...
        >
          <ModalHeader toggle={this.toggle}>
            {" "}
            Day Wise Bills for {monthName} - {year}{" "}
          </ModalHeader>
          <ModalBody>{this.dayWiseBill(history, monthData)}</ModalBody>
        </Modal>
      </div>
    );
  }
